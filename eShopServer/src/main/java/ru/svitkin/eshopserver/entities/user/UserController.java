...
public class UserController {
    ...
    @ApiResponse(responseCode = "200", content = {@Content(array = @ArraySchema(schema = @Schema(implementation = UserOutputDto.class)))})
    public List<UserOutputDto> getAll() {
        return userService.getAll()
                .stream()
                .map(user -> new UserOutputDto(user.getUsername(),
                        user.getEmail(),
