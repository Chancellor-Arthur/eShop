...
public class DeviceService {
	...
	public Device create(DeviceInputDto deviceInputDto) {
		...
		Device device = new Device(deviceInputDto.getName(), deviceInputDto.getPrice(), deviceInputDto.getImage(), type,
				brand);
	...
	public List<DeviceOutputDto> getAll() {
		return deviceRepository.findAll().stream().map(device -> new ModelMapper().map(device, DeviceOutputDto.class))
				.toList();
	}
